...
async function timedChunkSpeech(item,modelText){
  ...
  try{
    // Ask for / activate the microphone BEFORE the countdown.
    stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});
    cd.textContent="マイクOK";
    await new Promise(r=>setTimeout(r,250));
    for(let n=3;n>=1;n--){cd.textContent=n;await new Promise(r=>setTimeout(r,1000))}
    cd.textContent=`GO!  ${target.toFixed(1)}秒`;
    ...
    let leftTimer=setInterval(()=>{
      ...
      cd.textContent=`残り ${Math.max(0,target-elapsed).toFixed(1)}秒`;
    },100);

    // Give a small grace period so the final word is not clipped.
    timer=setTimeout(()=>{if(rec.state==="recording")rec.stop()},(target+0.7)*1000);
    await stopped;
    clearInterval(leftTimer);clearTimeout(timer);
    stream.getTracks().forEach(t=>t.stop());

    const elapsed=Math.min(target+0.7,(performance.now()-started)/1000);
    const blob=new Blob(chunks,{type:rec.mimeType||preferred||"audio/webm"});
    if(blob.size<800)throw new Error("録音データが空です");
    const audio=await blobToBase64(blob);
    cd.textContent="英語を聞き取っています…";
    const tr=await api("chunk-transcribe",{audio,mime:blob.type});
    const judge=await api("chunk-speech-check",{target:modelText,expression:item.expression,transcript:tr.text||"",elapsed:Math.min(elapsed,target),targetSeconds:target});
    return {...judge,transcript:tr.text||"",elapsed:Math.min(elapsed,target),targetSeconds:target};
  }catch(e){
    try{clearTimeout(timer);if(rec?.state==="recording")rec.stop();stream?.getTracks().forEach(t=>t.stop())}catch{}
    throw e;
...
async function transcribeOneAnswer(statusEl){
  ...
  try{
    ...
    const stopped=new Promise((resolve,reject)=>{rec.onstop=resolve;rec.onerror=e=>reject(e.error||e)});
    rec.start(250);
    setTimeout(()=>{if(rec.state==="recording")rec.stop()},9000);
    await stopped;stream.getTracks().forEach(t=>t.stop());
    statusEl.textContent="認識中…";
    const blob=new Blob(chunks,{type:rec.mimeType||preferred||"audio/webm"});
    const audio=await blobToBase64(blob);
    const tr=await api("chunk-transcribe",{audio,mime:blob.type});
    statusEl.textContent=tr.text?`認識：${tr.text}`:"聞き取れませんでした。もう一度どうぞ。";
    return tr.text||"";
  }catch(e){try{stream?.getTracks().forEach(t=>t.stop())}catch{};statusEl.textContent="音声認識に失敗しました。再試行またはスキップできます。";return""}
}
