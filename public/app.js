const APP_VERSION="1.11-realtime-english-only";
...
function looksEnglishPracticeText(text){
  const s=String(text||"").trim();
  if(!s)return false;
  const latin=(s.match(/[A-Za-z]/g)||[]).length;
  const jp=(s.match(/[\u3040-\u30ff\u3400-\u9fff]/g)||[]).length;
  return latin>=2 && latin>jp*2;
}
function normalizeCustomChunk(x){
  if(!x)return x;
  if(!looksEnglishPracticeText(x.expression) && looksEnglishPracticeText(x.seedExample)){
    return {...x,expression:x.seedExample};
  }
  return x;
}
function allChunkItems(){
  syncHighFrequencyCustom();
  const personal=(profile.chunkCustom||[]).map(normalizeCustomChunk).filter(x=>looksEnglishPracticeText(x.expression));
  return [...personal,...CHUNK_BANK];
}
function canonicalLessonModel(item){
  const l=item?.lesson||{};
  const candidates=[l.canonicalEnglish,l.example,item.seedExample,item.expression];
  return candidates.find(looksEnglishPracticeText)||item.expression;
}
function canonicalLessonJa(item){
  const l=item?.lesson||{};
  return String(l.canonicalJa||l.exampleJa||l.speakingPromptJa||item.meaning||"").trim();
}
function playEnglishOnly(text){
  if(!looksEnglishPracticeText(text))return toast("英語の模範文を確認できないため、音声再生を止めました");
  return playTTS(text);
}
...
async function startChunkCourse(kind="mixed",review=false){
  ...
  try{
    ...
    const d=await api("chunk-lessons",{theme,items:payload});
    const byId=Object.fromEntries((d.lessons||[]).map(x=>[x.id,x]));
    const lockedItems=items.map(x=>{
      const raw=byId[x.id]||{};
      let canonicalEnglish=String(raw.canonicalEnglish||raw.example||x.seedExample||x.expression||"").trim();
      if(!looksEnglishPracticeText(canonicalEnglish)) canonicalEnglish=looksEnglishPracticeText(x.seedExample)?x.seedExample:x.expression;
      const canonicalJa=String(raw.canonicalJa||raw.exampleJa||raw.speakingPromptJa||x.meaning||"").trim();
      const lesson={
        ...raw,
        canonicalEnglish,
        canonicalJa,
        example:canonicalEnglish,
        exampleJa:canonicalJa,
        speakingPromptJa:canonicalJa
      };
      return {...x,lesson};
    });
    state.chunks={active:true,theme,kind,review,items:lockedItems,index:0,step:1,writing:"",writingResult:null,results:[]};renderChunkLesson();
...
async function recordUntilSpeechEnds({targetSeconds=8,onFinishTime=null,maxExtra=.7}={}){
  let stream,rec,audioCtx,source,analyser,raf,hardStop,chunks=[];
  try{
    stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});
    const preferred=["audio/mp4","audio/webm;codecs=opus","audio/webm"].find(x=>window.MediaRecorder?.isTypeSupported?.(x));
    ...
    const rms=()=>{analyser.getByteTimeDomainData(data);let sum=0;for(const v of data){const x=(v-128)/128;sum+=x*x}return Math.sqrt(sum/data.length)};
    let ns=[];for(let i=0;i<8;i++){ns.push(rms());await new Promise(r=>setTimeout(r,35))}
    const noise=Math.max(.006,ns.reduce((a,b)=>a+b,0)/ns.length);
    const startThreshold=Math.max(.018,noise*2.8),keepThreshold=Math.max(.012,noise*1.8);

    rec.start(200);
    const goAt=performance.now();
    let speechStarted=false,lastVoiceAt=0,finished=false;
    const stopNow=()=>{if(finished)return;finished=true;cancelAnimationFrame(raf);clearTimeout(hardStop);if(rec.state==="recording")rec.stop()};
    hardStop=setTimeout(stopNow,(targetSeconds+maxExtra)*1000);
    const tick=()=>{
      if(finished)return;
      const now=performance.now(),level=rms();
      if(!speechStarted){
        if(level>=startThreshold){speechStarted=true;lastVoiceAt=now}
      }else{
        if(level>=keepThreshold)lastVoiceAt=now;
        if(now-lastVoiceAt>=700)stopNow();
      }
    ...
    const trailing=speechStarted?Math.min(.7,(stopAt-lastVoiceAt)/1000):0;
    const elapsed=Math.max(0,Math.min(targetSeconds+maxExtra,(stopAt-goAt)/1000-trailing));
    onFinishTime?.(elapsed,targetSeconds,speechStarted);
    const blob=new Blob(chunks,{type:rec.mimeType||preferred||"audio/webm"});
    if(blob.size<800)throw new Error("録音データが空です");
    return {blob,elapsed,targetSeconds,speechStarted};
  }finally{
    try{cancelAnimationFrame(raf)}catch{};try{clearTimeout(hardStop)}catch{};
    try{source?.disconnect();analyser?.disconnect()}catch{};try{await audioCtx?.close()}catch{};
    try{stream?.getTracks().forEach(t=>t.stop())}catch{}
  }
}

async function timedChunkSpeech(item,modelText){
  const target=chunkTargetSeconds(modelText),cd=$("#chunkCD"),btn=$("#chunkCount");
  btn.disabled=true;
  try{
    const pre=await navigator.mediaDevices.getUserMedia({audio:true});pre.getTracks().forEach(t=>t.stop());
    cd.textContent="マイクOK";await new Promise(r=>setTimeout(r,200));
    for(let n=3;n>=1;n--){cd.textContent=n;await new Promise(r=>setTimeout(r,1000))}
    cd.textContent=`GO! 目標 ${target.toFixed(1)}秒`;
    const result=await recordUntilSpeechEnds({
      targetSeconds:target,
      onFinishTime:(elapsed,goal,started)=>{cd.textContent=started?`⏱ ${elapsed.toFixed(1)}秒 / 目標 ${goal.toFixed(1)}秒`:`音声を検出できませんでした / 目標 ${goal.toFixed(1)}秒`}
    });
    const timeLine=cd.textContent;
    const audio=await blobToBase64(result.blob);
    const tr=await api("chunk-transcribe",{audio,mime:result.blob.type});
    cd.textContent=timeLine+(tr.text?`　🎤 ${tr.text}`:"　🎤 聞き取れませんでした");
    const judge=await api("chunk-speech-check",{target:modelText,expression:item.expression,transcript:tr.text||"",elapsed:result.elapsed,targetSeconds:target});
    return {...judge,transcript:tr.text||"",elapsed:result.elapsed,targetSeconds:target};
  }catch(e){cd.textContent="音声認識に失敗しました。再試行またはスキップできます。";throw e}
...
async function startRealtime(){if(missionConnected)return;const m=state.mission;transcript=[];currentAssistant="";missionFinishing=false;$("#startRealtime").disabled=true;setVoice("connecting","接続しています…","最初だけマイク許可が必要です");try{
    ...
    mic=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});pc=new RTCPeerConnection();pc.ontrack=e=>{remoteAudio.srcObject=e.streams[0];remoteAudio.play().catch(()=>{})};mic.getTracks().forEach(t=>pc.addTrack(t,mic));dc=pc.createDataChannel("oai-events");dc.onopen=()=>{
      ...
      dc.send(JSON.stringify({
        ...
        response:{instructions:`ENGLISH ONLY. Say the following line exactly in English. Do not translate it into Japanese. Do not add any Japanese before or after it. Say nothing else: ${JSON.stringify(opener)}`}
      }));
...
async function transcribeOneAnswer(statusEl){
  try{
    statusEl.textContent="🎙️ 話してください…";
    const result=await recordUntilSpeechEnds({targetSeconds:12,maxExtra:0,onFinishTime:(elapsed,goal,started)=>{statusEl.textContent=started?`⏱ ${elapsed.toFixed(1)}秒　認識中…`:"音声を検出できませんでした"}});
    const timeText=statusEl.textContent;
    const audio=await blobToBase64(result.blob);
    const tr=await api("chunk-transcribe",{audio,mime:result.blob.type});
    statusEl.textContent=tr.text?`${timeText.replace("認識中…","")}🎤 認識：${tr.text}`:"聞き取れませんでした。再試行またはスキップできます。";
    return tr.text||"";
  }catch(e){statusEl.textContent="音声認識に失敗しました。再試行またはスキップできます。";return""}
}
function renderChunkLesson(){
  ...
  else if(s.step===2){const model=canonicalLessonModel(item),promptJa=canonicalLessonJa(item);view.innerHTML=head+`<div class="card"><h3>${esc(item.meaning)}</h3><div class="en">${esc(model)}</div><div class="jp">${esc(promptJa)}</div><button id="clExampleHear" class="audioBtn">🔊 例文を聞く</button>${l.secondExample?`<div class="weakrow"><b>別の場面</b><div>${esc(l.secondExample)}</div></div>`:""}${l.tip?`<p class="note">💡 ${esc(l.tip)}</p>`:""}<button id="clWriteNext" class="primary wide">自分で1文作る</button></div>`;}
  else if(s.step===3)view.innerHTML=head+`<div class="card"><h3>自分で作文</h3><div class="jp">${esc(l.writingPromptJa||"この表現を使って、自分のことを1文書いてください。")}</div><textarea id="chunkWriting" placeholder="${esc(item.expression)} を使って英文を作る">${esc(s.writing||"")}</textarea><div class="row"><button id="voiceWriting" class="secondary">🎙️ 音声で答える</button><button id="skipWriting" class="secondary">スキップ</button></div><div id="writingVoiceStatus" class="note"></div><button id="checkWriting" class="primary wide">チェック</button>${s.writingResult?`<div class="weakrow"><b>${s.writingResult.ok?"✅ 通じています":"🔧 ここだけ自然に"}</b><div class="note">${esc(s.writingResult.feedbackJa||"")}</div><div class="en">${esc(s.writingResult.natural||"")}</div></div><button id="toChunkSpeak" class="secondary wide">発話へ</button>`:""}</div>`;
  else if(s.step===4){const model=canonicalLessonModel(item),promptJa=canonicalLessonJa(item),target=chunkTargetSeconds(model);view.innerHTML=head+`<div class="card center"><h3>声でCLEAR</h3><div class="big">${esc(promptJa)}</div><p class="note">この日本語は、下の模範英文と1対1で固定されています。3・2・1の後に話してください。</p><div class="en">目標：${target}秒以内</div><button id="chunkCount" class="primary wide">🎙️ 3・2・1 → 話す</button><button id="chunkSkipSpeech" class="secondary wide" style="margin-top:8px">今回はスキップ → 次へ</button><div id="chunkCD" class="countdown"></div><div id="speechResult"></div><div id="chunkModel" class="hidden"><div class="en big">${esc(model)}</div><button id="chunkModelHear" class="audioBtn">🔊 模範を聞く</button><button id="chunkRetry" class="secondary wide" style="margin-top:8px">もう一度</button><button id="chunkNext" class="primary wide hidden" style="margin-top:8px">CLEAR → 次へ</button></div></div>`;}
  if(s.step===1){$("#clHear").onclick=()=>playEnglishOnly(item.expression);$("#clMeaning").onclick=()=>{s.step=2;renderChunkLesson()}}
  if(s.step===2){$("#clExampleHear").onclick=()=>playEnglishOnly(canonicalLessonModel(item));$("#clWriteNext").onclick=()=>{s.step=3;renderChunkLesson()}}
  if(s.step===3){
    $("#voiceWriting").onclick=async()=>{const b=$("#voiceWriting"),st=$("#writingVoiceStatus");b.disabled=true;const text=await transcribeOneAnswer(st);if(text){$("#chunkWriting").value=text;s.writing=text}b.disabled=false};
    $("#skipWriting").onclick=()=>{s.writing="";s.writingResult={ok:true,feedbackJa:"作文をスキップしました。発話練習へ進みます。",natural:""};renderChunkLesson()};
  ...
  if(s.step===4){
    const model=canonicalLessonModel(item);
    const run=async()=>{try{const r=await timedChunkSpeech(item,model),p=chunkProg(item.id);p.attempts=(p.attempts||0)+1;p.lastTranscript=r.transcript;p.bestTime=p.bestTime==null?r.elapsed:Math.min(p.bestTime,r.elapsed);saveChunkProg(item.id,p);$("#chunkModel").classList.remove("hidden");const pass=!!r.ok&&!!r.withinTime;$("#speechResult").innerHTML=`<div class="weakrow"><b>${pass?"✅ CLEAR":"🔁 もう一度"}</b><div>認識：${esc(r.transcript||"（認識できませんでした）")}</div><div class="note">${r.elapsed.toFixed(1)}秒 / 目標 ${r.targetSeconds.toFixed(1)}秒　${esc(r.feedbackJa||"")}</div></div>`;$("#chunkNext").classList.toggle("hidden",!pass);if(pass)$("#chunkNext").onclick=()=>finishChunkItem(true);$("#chunkRetry").onclick=run;$("#chunkCount").disabled=false}catch(e){toast("音声認識に失敗しました。再試行するか、スキップできます");$("#chunkCount").disabled=false}};
    $("#chunkCount").onclick=run;$("#chunkSkipSpeech").onclick=()=>finishChunkItem(false);$("#chunkModelHear").onclick=()=>playEnglishOnly(model);$("#chunkRetry").onclick=run;
  }
}
function finishChunkItem(ok){
  ...
  else addWeakness({type:"fluency",label:item.expression,better:canonicalLessonModel(item),ja:canonicalLessonJa(item)||item.meaning,original:"3秒で出なかった",severityHint:1},"chunks");
