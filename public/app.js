...
function renderChunkLesson(){
  ...
  else if(s.step===4){const model=l.example||item.expression,target=chunkTargetSeconds(model);view.innerHTML=head+`<div class="card center"><h3>声でCLEAR</h3><div class="big">${esc(l.speakingPromptJa||item.meaning)}</div><p class="note">3・2・1の後に話してください。マイクが内容と時間を確認します。</p><div class="en">目標：${target}秒以内</div><button id="chunkCount" class="primary wide">🎙️ 3・2・1 → 話す</button><button id="chunkSkipSpeech" class="secondary wide" style="margin-top:8px">今回はスキップ → 次へ</button><div id="chunkCD" class="countdown"></div><div id="speechResult"></div><div id="chunkModel" class="hidden"><div class="en big">${esc(model)}</div><button id="chunkModelHear" class="audioBtn">🔊 模範を聞く</button><button id="chunkRetry" class="secondary wide" style="margin-top:8px">もう一度</button><button id="chunkNext" class="primary wide hidden" style="margin-top:8px">CLEAR → 次へ</button></div></div>`;}
  ...
  if(s.step===4){
    ...
    $("#chunkCount").onclick=run;$("#chunkSkipSpeech").onclick=()=>finishChunkItem(false);$("#chunkModelHear").onclick=()=>playTTS(model);$("#chunkRetry").onclick=run;
  }
}
