...
For each item, create ONE canonical spoken target sentence and ONE exact Japanese prompt for that SAME sentence.
The app will use this single pair for:
1) what the learner sees,
2) model audio,
3) speech recognition grading,
4) retry feedback.
Therefore canonicalEnglish and canonicalJa MUST be exact semantic equivalents. Never create a different sentence for speaking practice.

The item list has already been selected by corpus/usefulness priority. Do NOT invent frequency ranks.
canonicalEnglish MUST be English only, short, natural, reusable, and suitable for spoken communication.
canonicalJa MUST be Japanese only and must mean the same thing as canonicalEnglish.
canonicalEnglish MUST naturally contain/use the target expression or phrasal verb.
For phrasal verbs with many meanings, teach ONLY the supplied target meaning.
Personalize to the requested theme when natural.
Return JSON only.`,
...
各itemについて:
- id: 入力と同じ
- canonicalEnglish: このレッスンで唯一の正解英文。英語のみ
- canonicalJa: canonicalEnglishの意味と1対1で一致する自然な日本語
- secondExample: 別場面の短い英語例文
- writingPromptJa: 学習者自身の英作文用のお題。canonicalJaと別でもよい
- roleplayScene: Realtime会話でその表現を自然に使えそうな場面
- tip: 使い方の注意を日本語で1文

禁止:
- canonicalEnglish と canonicalJa の内容を変える
- canonicalJa に、canonicalEnglishにない情報を足す
- canonicalEnglish に日本語を混ぜる
- canonicalJa に英語の別正解文を混ぜる
- speaking用に別の英文を作る

JSON:
{"lessons":[{"id":"...","canonicalEnglish":"...","canonicalJa":"...","secondExample":"...","writingPromptJa":"...","roleplayScene":"...","tip":"..."}]}`
})));
...
app.post("/api/chunk-speech-check", safe(async b => askJSON({
  model:FAST_MODEL,
  instructions:`Judge a Japanese English learner's spoken attempt against the ONE canonical target supplied in Target model. Never substitute, invent, or prefer a different example sentence. The goal is fluent retrieval, not accent perfection. Accept small article, contraction, or transcription differences when the same intended target meaning is clearly communicated. Return JSON only.`,
...
- The user may describe the scene, role, missions, or control commands in Japanese. Understand that Japanese normally.
- Your SPOKEN roleplay output must be NATURAL ENGLISH ONLY.
- Never speak Japanese during the roleplay, including the very first line.
- Never translate your English reply into Japanese.
- Never repeat Japanese scene text or mission text aloud.
- Japanese is input/context only. It is never the spoken output language of the roleplay.
- If the user says a Japanese control command such as 「ゆっくり」「もう一度」「止めて」「ちょっと」, obey the command but continue speaking only in English.
- If the user speaks Japanese as part of the roleplay, respond in English as the real-world counterpart unless they are clearly giving a control command.
...
ROLEPLAY FLOW:
1. The app supplies an ENGLISH opening line. Say/continue it in English exactly as intended. Never translate the opening line into Japanese. Continue from it as the real-world role; never introduce yourself as a language buddy, teacher, guide, or learning assistant.
